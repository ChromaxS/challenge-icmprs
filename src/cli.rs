...
pub(crate) struct Args {
    #[arg(
        required = true,
        help = "Host (or address) or CSV format: host,count,interval (implies --output=csv)"
    )]
...
pub fn count_range(count_parsed: &str) -> Result<u64, anyhow::Error> {
    ...
    match count_parsed.parse() {
        ...
        _ => Err(anyhow::anyhow!(
            "The count must be a value between 1 and 10: {}",
            count_parsed
        )),
    }
}
...
pub fn interval_range(interval_parsed: &str) -> Result<u64, anyhow::Error> {
    ...
    match interval_parsed.parse() {
        ...
        _ => Err(anyhow::anyhow!(
            "The interval must be a value between 1 and 1000ms: {}",
            interval_parsed
        )),
    }
}
...
pub fn timeout_range(timeout_parsed: &str) -> Result<u64, anyhow::Error> {
    ...
    match timeout_parsed.parse() {
        ...
        _ => Err(anyhow::anyhow!(
            "The timeout must be a value between 1 and 10000ms: {}",
            timeout_parsed
        )),
    }
}
