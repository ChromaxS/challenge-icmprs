...
pub(crate) fn ipv4_handle_packet(
    ...
    if ip_version != D_IP_IP_VERSION_4 {
        return Err(anyhow::anyhow!(
            "Invalid IPv4 header version, {} != {}, from: {}",
            ip_version,
            D_IP_IP_VERSION_4,
            remote_addr
        ));
    ...
    log::debug!(
        "got ipv4 packet: {:?} (data_offset={}, data_len={})",
        &ip,
        data_offset.to_be(),
        ip.length.to_be()
    );
    ...
    match ip.protocol {
        D_IP_IP_TYPE_ICMP => {
            icmp::ipv4_handle_icmp_packet(remote_sock, &ip, &data.to_vec(), program_state)
        }
